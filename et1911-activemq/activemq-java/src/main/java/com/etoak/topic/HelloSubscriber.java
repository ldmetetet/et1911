 ...
 * session.createTopic("Hi topic"); MessageConsumer consumer =
 * session.createConsumer(topic); consumer.setMessageListener(new
 * MessageListener() {
 * 
 * @Override public void onMessage(Message message) { // TODO Auto-generated
 * method stub if(message instanceof MapMessage) { MapMessage
 * msg=(MapMessage)message;
 * 
 * try { int id = msg.getInt("id"); String name = msg.getString("name");
 * System.out.println(id +'-'+name);
 * 
